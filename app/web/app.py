...
from .websocket import register_websocket_handler # Assuming this calls app.add_websocket_route or uses @app.websocket
...
def create_app() -> FastAPI:
    ...
    app.add_middleware(
        CORSMiddleware,
        allow_origins=["*"],  # e.g., ["http://localhost:3000", "https://yourfrontend.com"]
    ...
    app.add_middleware(TrustedHostMiddleware, allowed_hosts=["*", "localhost", "127.0.0.1"]) # Be more specific in prod
    ...
    try:
        ...
        logger.error(f"Failed during WebSocket handler registration: {e}", exc_info=True)
