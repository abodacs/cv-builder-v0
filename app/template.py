...
SCRTIPT = """ 
 <script>
    ...
    // Ensure protocol is secure (wss) if the page is served over https
    const wsProtocol = window.location.protocol === "https:" ? "wss://" : "ws://";
    const ws = new WebSocket(wsProtocol + window.location.host + "/ws/cv_builder");

    ws.onopen = function() {
        console.log("WebSocket connection established.");
        // Send session ID and initial language on connect
        ws.send(JSON.stringify({
            session_id: sessionId,
            text: "", // Initial message can be empty
            language: currentLanguage,
            is_initial: true // Optional flag to tell server it's the first connection message
        }));
    };
    ...
    ws.onerror = function(error) {
        ...
        addSystemMessage("Connection error. Please check your connection and refresh.");
    };
